...
| 摄像头驱动 | src/support/driver/camera_driver.py | Orbbec Astra Pro 深度相机已调通，支持 JPEG 流 |
...
| 摄像头 | Orbbec Astra Pro 深度相机，/dev/video0，640x480 | tests/camera/ |
...
IMU（BNO055）已调通，输出四元数姿态。Orbbec Astra Pro 深度相机流已调通。RPLIDAR S1 串口通信验证通过，待封装为 ROS2 LaserScan 话题。注意：当前 lidar_driver.py 为 Mock 数据，M0 阶段优先替换为真实驱动。
...
| 摄像头 | Orbbec Astra Pro 深度相机，/dev/video0 | 已调通 | 640x480，MJPEG 流 |
