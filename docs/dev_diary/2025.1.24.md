...
*   [x] **SLAM 专项突破** (Ref: `docs/design/slam_fusion_roadmap.md`)
    *   [x] **路线图规划**: 完成 SLAM 选型与 Roadmap 设计 (RTAB-Map vs Cartographer)
...
## 🚀 6. 明日计划 (Next Steps)
*   [ ] **数据准备**: 检查 IMU/LiDAR/Camera 频率与时间同步。
*   [ ] **TF 验证**: 使用 `view_frames` 确认 TF 树结构正确。
*   [ ] **里程计融合**: 配置 `robot_localization` 并验证 odom 精度。
*   [ ] **SLAM 离线测试**: 录制 Rosbag 并在 PC 上跑通 RTAB-Map 建图。
