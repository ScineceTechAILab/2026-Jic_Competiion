...
REG_M4_ENCODER = 0x13      # 读取M4编码器实时脉冲数据 (只读)
...
def main():
    ...
    def read_m4_encoder():
        try:
            ...
            if len(data) >= 2:
                ...
                print(f"[状态] M4编码器实时脉冲值: {encoder_value}")
                return encoder_value
            else:
                print("[错误] 读取M4编码器失败：数据长度不足")
                return None
        except OSError as e:
            print(f"[错误] 读取M4编码器失败: {e}")
            return None

    # 3. 通过PWM控制M4电机（其他电机保持为0）
    # 注意：根据文档，I2C控制必须一次性写入4个电机的数据
    def set_m4_pwm(pwm_value):
        ...
        data_bytes.extend(struct.pack('>h', 0))   # M3 PWM = 0
        data_bytes.extend(struct.pack('>h', int(pwm_value))) # M4 PWM = 设定值
        
        try:
            # 写入PWM控制寄存器 (0x07)
            bus.write_i2c_block_data(DEVICE_ADDR, REG_PWM_CONTROL, list(data_bytes))
            print(f"[控制] 设置M4 PWM值: {pwm_value} (范围: -3600 ~ 3600)")
            return True
        except OSError as e:
            print(f"[错误] I2C 写入失败: {e}")
            return False

    # 4. 通过速度控制M4电机（仅对带编码器的电机有效）
    def set_m4_speed(speed_value):
        ...
        data_bytes.extend(struct.pack('>h', 0))   # M3 速度 = 0
        data_bytes.extend(struct.pack('>h', int(speed_value))) # M4 速度 = 设定值
        
        try:
            # 写入速度控制寄存器 (0x06)
            bus.write_i2c_block_data(DEVICE_ADDR, REG_SPEED_CONTROL, list(data_bytes))
            print(f"[控制] 设置M4速度值: {speed_value} (范围: -1000 ~ 1000)")
    ...
    def config_m4_motor():
        try:
            ...
            # 注意：文档中REG_MAGNETIC_LINES为uint16_t，但例子是单字节。此处按uint16_t处理。
    ...
    try:
        ...
        # 步骤2: （可选）配置电机参数
        # config_m4_motor() # 取消注释以启用配置
        
        # 步骤3: 读取M4编码器初始值
        print("\n[步骤2] 读取M4编码器初始值...")
        initial_encoder = read_m4_encoder()
        
        # 步骤4: PWM控制测试 - 正向转动
        print("\n[步骤3] PWM控制测试 - M4正向转动")
        print("        预计M4电机（右下轮）会正向转动")
        if set_m4_pwm(1500):
        ...
        # 步骤5: PWM控制测试 - 反向转动（倒退）
        print("\n[步骤4] PWM控制测试 - M4反向转动（倒退）")
        print("        预计M4电机（右下轮）会反向转动")
        if set_m4_pwm(-1500):
            print("        PWM指令发送成功，等待3秒...")
            time.sleep(3)
            read_m4_encoder()
