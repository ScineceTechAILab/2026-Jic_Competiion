...
REG_M3_ENCODER = 0x14      # 读取M3编码器数据 (只读)
REG_M4_ENCODER = 0x16      # 读取M4编码器数据 (只读) - 舵机位置反馈
...
def main():
    ...
    print("测试范围：M2电机（左下/左后轮）和 M4舵机（转向）")
    ...
    # 只控制M4舵机的PWM（其他电机保持为0）
    # 舵机通常使用PWM信号控制，范围通常为 -900 到 900（中心位置为0）
    def set_m4_servo_pwm(pwm_value):
        # 限制舵机PWM范围: -900 到 900（对应舵机的旋转范围）
        if pwm_value > 900: 
            pwm_value = 900
        elif pwm_value < -900: 
            pwm_value = -900
        
        # 转换为大端模式的int16_t数组（只设置M4，其他为0）
        pwm_int = int(pwm_value)
        data_bytes = bytearray()
        ...
        try:
            # 写入PWM控制寄存器 (0x07)
            bus.write_i2c_block_data(DEVICE_ADDR, REG_PWM_CONTROL, list(data_bytes))
            print(f"设置M4舵机 PWM值: {pwm_value}")
            return True
        except OSError as e:
            print(f"I2C 写入失败: {e}")
            return False

    # 读取M4舵机位置反馈（根据文档0x16寄存器）
    def read_m4_servo_position():
        try:
            # 读取M4舵机位置寄存器 (0x16)
            ...
            if len(data) >= 2:
                ...
                print(f"M4舵机位置值: {position_value}")
                return position_value
            else:
                print("读取M4舵机位置失败：数据长度不足")
                return None
        except OSError as e:
            print(f"读取M4舵机位置失败: {e}")
            return None

    # 只控制M4舵机的速度（其他电机保持为0）
    # 用于带编码器的舵机进行速度控制
    def set_m4_servo_speed(speed_value):
        # 限制舵机速度范围: -500 到 500
        if speed_value > 500: 
            speed_value = 500
    ...
    def config_m4_servo():
        try:
            ...
            # 根据文档：舵机可能使用特定的类型配置
            print("1. 配置M4为舵机类型...")
            bus.write_byte_data(DEVICE_ADDR, REG_MOTOR_TYPE, 4)  # 假设4为舵机类型
            print("  舵机类型配置发送完成")
            time.sleep(0.5)
